...
//图片读取与显示
function handleFiles(file,id,img_file,width,height){
var docObj=document.getElementById(id); 
var imgObjPreview=document.getElementById(img_file);
...
function onload_vue(){
	var vue_1 = new Vue({
		...
		data:{
			items: [
			 {y:'2017,2-2017,9',u:'北京理工大学珠海学院',w:'前端开发',m:'负责收集产品相关信息和新闻信息进行日常更新与维护'} 
			],
			items_1:[
			 '2012-2013学年获得广东五百丁学院优秀学生；' ,
             '四川大学优秀团员 四川大学单项一等奖学金（社会实践）2次；' ,
             '2012-2013年院学生会十佳干事' 
			],
			items_2:[
			{y:'2017,2-2017,9',u:'北京理工大学珠海学院',w:'软件工程（学士）',m:'计算机应用基础、算法、数据库、数据结构、计算机组成、C++、C#。'}
			],
			items_3:[
			{y:'2017,5-2017,10',u:'填写项目名称',w:'填写角色名称',m:'在这里添加描述......'}
			],
			items_4:[
			{src:'../img/1-130412092606.jpg',n:'填写作品名称',m:'填写作品描述'}
			]
		},
		methods: {
        push_one: function (ps_one) {         		
        			if(ps_one==0){
        			...
        			if(ps_one==1){
        				title = propt(); 
        				if(title!=0){
        				 this.items_1.push(title);	
        				}        				 
        			}	
        			if(ps_one==2){
        				this.items_2.push(this.items_2[0]);
        			}
        			if(ps_one==3){
        				this.items_3.push(this.items_3[0]);
        			}
           },
        delete_one: function(det_one){
        	if(det_one==0){        		
        	...
        	if(det_one==1){
        		if(this.items_1.length>1){
        			this.items_1.pop();
        		}
        	}
        	if(det_one==2){
        		if(this.items_2.length>1){
        			this.items_2.pop();
        		}
        	}
        	if(det_one==3){
        		if(this.items_3.length>1){
        			this.items_3.pop();
        		}
        	}
        }
	...
	var vue_2 = new Vue({
		el:'#sec_3_main_div_id',
		data:{
			data_1:[
			{id:0,name:'HTML5/css',wid:"90%",hao:"精通"},
			{id:1,name:'javascript',wid:"80%",hao:"熟练"},
			{id:2,name:'jquery',wid:"60%",hao:"了解"}
			],
			data_2:[
			{id:4,name:'电影',src:"../img/int/4.png"},
			{id:5,name:'旅游',src:"../img/int/5.png"},
			{id:6,name:'举重',src:"../img/int/6.png"},
			{id:7,name:'烹饪',src:"../img/int/7.png"},
			{id:8,name:'羽毛球',src:"../img/int/8.png"},
			{id:10,name:'篮球',src:"../img/int/10.png"},
			{id:11,name:'电子游戏',src:"../img/int/11.png"},
			{id:13,name:'阅读',src:"../img/int/13.png"},
			{id:14,name:'台球',src:"../img/int/14.png"},
			{id:15,name:'户外运动',src:"../img/int/15.png"}
			],
			data_3:[
			 {id:0,name:'摄影',src:"../img/int/0.png"},
			 {id:1,name:'骑行',src:"../img/int/1.png"},
			 {id:2,name:'音乐',src:"../img/int/2.png"},
			 {id:3,name:'健身',src:"../img/int/3.png"},
			]
		},
		methods:{
			push_one: function(){
				var id = this.data_1.length;
				var name = $('#input_id').val();
				var hao = $('#select_id option:selected' ).text();
				var wid = $('#select_id').val();
				row = {id:id,name:name,wid:wid,hao:hao} ;
				this.data_1.push(row);
			},
			delete_one:function(one){
				for(var i =0;i<=this.data_1.length;i++){
					if(this.data_1[i].id==one){
						this.data_1.splice(i,1);
					}
				}				
			},
			add_one:function(one){
				for(var i =0;i<=this.data_2.length;i++){
					if(this.data_2[i].id==one){
						this.data_3.push(this.data_2[i]);
						this.data_2.splice(i,1);
					}
				}
			},
			cut_one:function(one){
				for(var i =0;i<=this.data_3.length;i++){
					if(this.data_3[i].id==one){
						this.data_2.push(this.data_3[i]);
						this.data_3.splice(i,1);
					}
				}
			}
		}
	...
	var vue_3 = new Vue({
	 el:'#sec_3_right_div_id',
	 data:{
	 	data1:[
	 	  {id:0,name:'求职意向',gly:'glyphicon-picture',seen:true},
	 	  {id:1,name:'教育背景',gly:'glyphicon-bookmark',seen:true},
	 	  {id:2,name:'实习经验',gly:'glyphicon-tags',seen:true},
	 	  {id:3,name:'荣誉奖项',gly:'glyphicon-ok',seen:true},
	 	  {id:4,name:'自我评价',gly:'glyphicon-star-empty',seen:true},
	 	  {id:5,name:'项目经验',gly:'glyphicon-retweet',seen:false},
	 	  {id:6,name:'作品展示',gly:'glyphicon-plane',seen:false}
	 ...
	 methods:{
	 	add_one:function(one){
	 		for(var i =0;i<=this.data1.length;i++){
					if(this.data1[i].id==one){						
						this.data1[i].seen=true;
						var div_id = "sec_3_main_right_div_" + i;
						var div_id_1 = "phone_div_" + i;
						document.getElementById(div_id).style.display="block";
						document.getElementById(div_id_1).style.display="block";
					}
				}
	 	},
	 	cut_one:function(one){
...
//预览图片
function get_img(){
	$('#resume_div').show(1000);
	$('#resume_img').css('height','1350px');
	html2canvas($('#resume_img'), {
                        allowTaint: true,  
                        taintTest: false,  
                        onrendered: function(canvas) { 
                            canvas.id = "mycanvas";  
                            var dataUrl = canvas.toDataURL();  
                             $('#img_3').attr('src',dataUrl); 
                             
                        }  
                   });
    $('#resume_img').css('height','870px')
}
//下载
function download_img(num) {
	if(num==0){
	$('#resume_img').css('height','1350px');
 	html2canvas($('#resume_img'), {  
                        allowTaint: true,  
                        ...
                        onrendered: function(canvas) { 
                            ...
                            $a.setAttribute("download", "个人简历.jpg");
...
function phone_vue_onload(){
	var vue_4 = new Vue({
        ...
        push_one: function (ps_one,id) {         		
        			...
        			if(ps_one==5){
        				var id = this.items_5.length;
				        var name = $('#input_id_phone').val();
				        var hao = $('#select_id_phone option:selected' ).text();
				        var wid = $('#select_id_phone').val();
				        row = {id:id,name:name,wid:wid,hao:hao} ;
				        this.items_5.push(row);
        			}
        			if(ps_one==6){
        				for(var i =0;i<=this.items_6.length;i++){
					    if(this.items_6[i].id==id){
						this.items_7.push(this.items_6[i]);
						this.items_6.splice(i,1);
					}
				}
        			}
           },
        delete_one: function(det_one,id){
        	if(det_one==0){        		
        		if(this.items.length>1){
        			this.items.pop();
        		}
        	...
        	if(det_one==5){
        		for(var i =0;i<=this.items_5.length;i++){
					if(this.items_5[i].id==id){
						this.items_5.splice(i,1);
					}
				}
        	}
        	if(det_one==6){
        		for(var i =0;i<=this.items_7.length;i++){
					if(this.items_7[i].id==id){
						this.items_6.push(this.items_7[i]);
						this.items_7.splice(i,1);
					}
				}
        	}
        }
